...
cnt=mysql.connector.connect(user='root',password='Ankit@143',host='localhost',database='fotofobia')
app = Flask(__name__)
...
def contact():
    if request.method=='POST':
        ...
        csr=cnt.cursor()
        # csr.execute(f"insert into contact values('{name}','{email}','{subject}','{message}');")
        csr.execute("insert into contact values(%s,%s,%s,%s)",(name,email,subject,message))
        cnt.commit()
